...
if __name__ == '__main__':
    ...
    for source_model in seed_countries:
        ...
        model = torch.load(f'../models/{DATA_OUT}/{source_model}_bal_gnn.pt')
        #optimizer = torch.optim.Adam(model.parameters(), lr=0.001, weight_decay=5e-4)

        target_countries = ['Bolivia', 'Colombia', 'Ecuador','Chile']
        #already did self-predictions
        ...
        for country_name in target_countries:
            ...
            tdf.to_csv(f'../results/{source_model}/{DATA_OUT}/{country_name}_train_predictions.csv', index = False)
            vdf.to_csv(f'../results/{source_model}/{DATA_OUT}/{country_name}_val_predictions.csv', index = False)
            test_df.to_csv(f'../results/{source_model}/{DATA_OUT}/{country_name}_test_predictions.csv', index = False)
